...
import mylar
from mylar import logger, db, updater, helpers, findcomicfeed, notifiers, rsscheck, Failed, filechecker, \
    auth32p, sabnzbd, nzbget, wwt, getcomics


def search_init(ComicName, IssueNumber, ComicYear, SeriesYear, Publisher, IssueDate, StoreDate, IssueID,
                AlternateSearch=None, UseFuzzy=None, ComicVersion=None, SARC=None, IssueArcID=None, mode=None,
                rsscheck=None, ComicID=None, manualsearch=None, filesafe=None, allow_packs=None, oneoff=False,
                manual=False, torrentid_32p=None, digitaldate=None, booktype=None, ignore_booktype=False):
    mylar.COMICINFO = []
    ...
    if mylar.CONFIG.ENABLE_TORRENT_SEARCH:  # and mylar.CONFIG.ENABLE_TORRENTS:
        ...
        if mylar.CONFIG.ENABLE_TORZNAB is True:
            for torznab_host in mylar.CONFIG.EXTRA_TORZNABS:
                if any([torznab_host[5] == '1', torznab_host[5] == 1]) and not helpers.block_provider_check(
                        torznab_host[0]):
    ...
    if mylar.CONFIG.NEWZNAB is True:
        for newznab_host in mylar.CONFIG.EXTRA_NEWZNABS:
            if any([newznab_host[5] == '1', newznab_host[5] == 1]) and not helpers.block_provider_check(
                    newznab_host[0]):
    ...
    if totalproviders == 0:
        logger.error(
            '[WARNING] You have %s search providers enabled. I need at least ONE provider to work. Aborting search.' % totalproviders)
        findit['status'] = False
        nzbprov = None
    ...
    while (srchloop <= searchcnt):
        ...
        while (cmloopit >= 1):
            ...
            while (tmp_prov_count > prov_count):
                ...
                if all([searchprov == 'dognzb', mylar.CONFIG.DOGNZB == 0]) or all(
                        [searchprov == 'dognzb', provider_blocked]):
                ...
                if searchmode == 'rss':
                    ...
                    #    prov_count+=1
                    #    continue
                    findit = NZB_SEARCH(ComicName, IssueNumber, ComicYear, SeriesYear, Publisher, IssueDate, StoreDate,
                                        searchprov, IssDateFix, IssueID, UseFuzzy, newznab_host,
                                        ComicVersion=ComicVersion, SARC=SARC, IssueArcID=IssueArcID, RSS="yes",
                                        ComicID=ComicID, oneoff=oneoff, cmloopit=cmloopit, manual=manual, torznab_host=torznab_host,
                                        digitaldate=digitaldate, booktype=booktype, chktpb=chktpb,
                                        ignore_booktype=ignore_booktype, c_number=c_number, c_alpha=c_alpha)
                    if findit['status'] is False:
                        if AlternateSearch is not None and AlternateSearch != "None":
                            ...
                            for calt in chkthealt:
                                ...
                                findit = NZB_SEARCH(AS_Alternate, IssueNumber, ComicYear, SeriesYear, Publisher,
                                                    IssueDate, StoreDate, searchprov, IssDateFix,
                                                    IssueID, UseFuzzy, newznab_host, ComicVersion=ComicVersion,
                                                    SARC=SARC, IssueArcID=IssueArcID, RSS="yes", ComicID=ComicID,
                                                    allow_packs=allow_packs, oneoff=oneoff, cmloopit=cmloopit,
                                                    manual=manual, torznab_host=torznab_host, digitaldate=digitaldate,
                                                    booktype=booktype, chktpb=chktpb, ignore_booktype=ignore_booktype,
                                                    c_number=c_number, c_alpha=c_alpha)
                                if findit['status'] is True:
                                    break
                            if findit['status'] is True:
                                break
                    ...
                    findit = NZB_SEARCH(ComicName, IssueNumber, ComicYear, SeriesYear, Publisher, IssueDate, StoreDate,
                                        searchprov, IssDateFix, IssueID, UseFuzzy, newznab_host,
                                        ComicVersion=ComicVersion, SARC=SARC, IssueArcID=IssueArcID, RSS="no",
                                        ComicID=ComicID, allow_packs=allow_packs, oneoff=oneoff, cmloopit=cmloopit, manual=manual,
                                        torznab_host=torznab_host, torrentid_32p=torrentid_32p, digitaldate=digitaldate,
                                        booktype=booktype, chktpb=chktpb, ignore_booktype=ignore_booktype)
                        ...
                        if AlternateSearch is not None and AlternateSearch != "None":
                            ...
                            for calt in chkthealt:
                                ...
                                findit = NZB_SEARCH(AS_Alternate, IssueNumber, ComicYear, SeriesYear, Publisher,
                                                    IssueDate, StoreDate, searchprov, IssDateFix,
                                                    IssueID, UseFuzzy, newznab_host, ComicVersion=ComicVersion,
                                                    SARC=SARC, IssueArcID=IssueArcID, RSS="no", ComicID=ComicID,
                                                    allow_packs=allow_packs, oneoff=oneoff, cmloopit=cmloopit,
                                                    manual=manual, torznab_host=torznab_host,
                                                    torrentid_32p=torrentid_32p, digitaldate=digitaldate,
                                                    booktype=booktype, chktpb=chktpb, ignore_booktype=ignore_booktype,
                                                    c_number=c_number, c_alpha=c_alpha)
                                if findit['status'] is True:
                                    break
                ...
                if manual is not True:
                    ...
                    if issuedisplay is None:
                        logger.info(
                            'Could not find %s (%s) using %s [%s]' % (ComicName, SeriesYear, searchprov, searchmode))
                    else:
                        logger.info('Could not find Issue %s of %s (%s) using %s [%s]' % (
                            issuedisplay, ComicName, SeriesYear, searchprov, searchmode))
                prov_count += 1
...
def NZB_SEARCH(ComicName, IssueNumber, ComicYear, SeriesYear, Publisher, IssueDate, StoreDate, nzbprov, IssDateFix,
               IssueID, UseFuzzy, newznab_host=None, ComicVersion=None, SARC=None, IssueArcID=None,
               RSS=None, ComicID=None, allow_packs=None, oneoff=False,
               cmloopit=None, manual=False, torznab_host=None, torrentid_32p=None, digitaldate=None, booktype=None,
               chktpb=0, ignore_booktype=False, c_number=None, c_alpha=None):
    if any([allow_packs is None, allow_packs == 'None', allow_packs == 0, allow_packs == '0']) and all(
            [mylar.CONFIG.ENABLE_TORRENT_SEARCH, mylar.CONFIG.ENABLE_32P]):
        allow_packs = False
    elif any([allow_packs == 1, allow_packs == '1']) and all(
            [mylar.CONFIG.ENABLE_TORRENT_SEARCH, mylar.CONFIG.ENABLE_32P]):
        allow_packs = True
    ...
    name_newznab = None
    host_newznab = None
    category_newznab = None
    name_torznab = None
    host_torznab = None
    category_torznab = None
    verify = True
    apikey = None
    host_newznab_fix = None
    ...
    if '0-Day Comics Pack' in ComicName:
        ...
        logger.info('Shhh be very quiet...I\'m looking for %s issue: %s (%s) using %s.' % (
    ...
    cm = re.sub("\\band\\b", "",
                cm1.lower())  # remove 'and' & '&' from the search pattern entirely (broader results, will filter out
    # later)
            ...
            if nzbprov == '32P':
                if all([mylar.CONFIG.MODE_32P == 1, mylar.CONFIG.ENABLE_32P is True]):
                    if ComicName[:17] == '0-Day Comics Pack':
                        ...
                        searchterm = {'series': ComicName, 'id': ComicID, 'issue': findcomiciss, 'volume': ComicVersion,
                                      'publisher': Publisher, 'torrentid_32p': torrentid_32p, 'booktype': booktype}
                ...
                if nzbprov == 'dognzb':
                    ...
                    findurl = str(host_newznab_fix) + "api?t=search&q=" + str(comsearch) + "&o=xml&cat=" + str(
                        category_newznab)
                elif nzbprov == 'torznab':
                    ...
                    logger.warn(
                        'You have a blank newznab entry within your configuration. Remove it, save the config and '
                        'restart mylar to fix things. Skipping this blank provider until fixed.')
                    findurl = None
                    bb = "noresults"
                ...
                if findurl:
                    ...
                    if mylar.CONFIG.USENET_RETENTION is not None and nzbprov != 'torznab':
                    ...
                    if nzbprov == 'newznab':
                        ...
                        if any([hnc[:3] == '10.', hnc[:4] == '172.', hnc[:4] == '192.', hnc.startswith('localhost'),
                                newznab_local is True]) and newznab_local != False:
                    ...
                    if findurl.startswith('https:') and verify == False:
                        try:
                            ...
                            logger.warn(
                                'Unable to disable https warnings. Expect some spam if using https nzb providers.')
                    ...
                    try:
                        if str(r.status_code) != '200':
                            logger.warn('Unable to retrieve search results from %s [Status Code returned: %s]' % (
                                tmpprov, r.status_code))
                    ...
                    try:
                        if bb == 'no results':
                            ...
                            logger.error('[ERROR CODE: %s] %s' % (
                                bb['feed']['error']['code'], bb['feed']['error']['description']))
        ...
        if not bb == "no results":
            for entry in bb['entries']:
                ...
                if nzbprov == '32P' and entry['title'][:17] == '0-Day Comics Pack':
                    ...
                    if entry['pack'] == '0':
                        if helpers.issuedigits(entry['issues']) == intIss:
                            ...
                            logger.fdebug(
                                'The search result issue [%s] does not match up for some reason to our search result [%s]' % (
                                    entry['issues'], findcomiciss))
                            continue
                    elif any([entry['pack'] == '1', entry['pack'] == '2']) and allow_packs is False:
                        if pack_warning is False:
                            logger.fdebug(
                                '(possibly more than one) Pack detected, but option not enabled for this series. Ignoring subsequent pack results (to enable: on the series details page -> Edit Settings -> Enable Pack Downloads)')
                            pack_warning = True
                        continue

                logger.fdebug("checking search result: %s" % entry['title'])
                # some nzbsites feel that comics don't deserve a nice regex to strip the crap from the header,
                # the end result is that we're dealing with the actual raw header which causes incorrect matches
                # below. this is a temporary cut from the experimental search option (findcomicfeed) as it does this
                # part well usually.
                ...
                for subs in splitTitle:
                    ...
                    try:
                        if len(subs) >= len(ComicName.split()) and not any(
                                d in subs.lower() for d in except_list) and bool(_digits.search(subs)) is True:
                            if subs.lower().startswith('for'):
                                if ComicName.lower().startswith('for'):
                                    ...
                                    continue
                                logger.fdebug(
                                    'Detected crap within header. Ignoring this portion of the result in order to see if it\'s a valid match.')
                            ComicTitle = subs
                            break
                    except:
                        break
                ...
                if nzbprov != "dognzb":
                    ...
                    if RSS == "yes":
                        if nzbprov == '32P':
                            try:
                                # newer rss feeds will now return filesize from 32p. Safe-guard it incase it's an
                                # older result
                        ...
                        if nzbprov == '32P':
                            ...
                            comsize_b = entry[
                                'length']  # we only want the size from the rss - the search/api has it already.
                        else:
                            try:
                                if entry['site'] == 'WWT':
                                    comsize_b = entry['size']
                    ...
                    # file restriction limitation here only works with TPSE (done here) & 32P (done in rsscheck) &
                    # Experimental (has it embeded in search and rss checks)
                    if nzbprov == 'Public Torrents' or (
                            nzbprov == '32P' and RSS == 'no' and entry['title'][:17] != '0-Day Comics Pack'):
                ...
                if UseFuzzy == "1":
                    ...
                    # use store date instead of publication date for comparisons since publication date is usually +2
                    # months
                    if StoreDate is None or StoreDate == '0000-00-00':
                        if IssueDate is None or IssueDate == '0000-00-00':
                            logger.fdebug(
                                'Invalid store date & issue date detected - you probably should refresh the series or wait for CV to correct the data')
                            continue
                        else:
                            stdate = IssueDate
                    ...
                    if any([postdate_int is None, type(postdate_int) != int]) or not all(
                            [nzbprov == '32P', RSS == 'no']):
                        ...
                        if all([nzbprov == '32P', dateconv is None, RSS == 'no']):
                            try:
                                ...
                                logger.warn('Unable to parsedate to a long-date that I can undestand : %s' % entry[
                                    'int_pubdate'])
                            else:
                        ...
                        try:
                            ...
                            logger.warn(
                                'Unable to parse posting date from provider result set for : %s' % entry['title'])
                            continue
                    ...
                    while i <= 1:
                        ...
                        # to get past different locale's os-dependent dates, let's convert it to a generic datetime
                        # format
                        try:
                            ...
                            logger.fdebug(
                                'Error attempting to convert the timestamp into a generic format. Probably due to the epoch limiation.')
                    ...
                    try:
                        # try new method to get around issues populating in a diff timezone thereby putting them in a
                        # different day. logger.info('digitaldate: %s' % digitaldate) logger.info('dateconv2: %s' %
                        # dateconv2.date()) logger.info('digconv2: %s' % digconv2.date())
                        if digitaldate != '0000-00-00' and dateconv2.date() >= digconv2.date():
                            ...
                            logger.fdebug(
                                '%s is before store date of %s. Ignoring search result as this is not the right issue.' % (
                                    pubdate, stdate))
                            continue
                        else:
                ...
                if '(digital first)' in ComicTitle.lower():  # entry['title'].lower():
                    dig_moving = re.sub('\(digital first\)', '',
                                        ComicTitle.lower()).strip()  # entry['title'].lower()).strip()
                ...
                logger.info('booktype: %s / parsed_booktype: %s [ignore_booktype: %s]' % (
                    booktype, parsed_comic['booktype'], ignore_booktype))
                if parsed_comic['parse_status'] == 'success' and (
                        all([booktype is None, parsed_comic['booktype'] == 'issue']) or all(
                    [booktype == 'Print', parsed_comic['booktype'] == 'issue']) or all(
                    [booktype == 'One-Shot', parsed_comic['booktype'] == 'issue']) or all(
                    [booktype != parsed_comic['booktype'], ignore_booktype is True]) or booktype == parsed_comic[
                            'booktype']):
                    try:
                        ...
                        if filecomic['process_status'] == 'fail':
                            ...
                            # if it's an alternate series match, we'll retain each value until the search has
                            # compeletely run, compiling matches. if at any point it's a standard match (ie.
                            # non-alternate series) that will be accepted as the one match and ignore the alts once
                            # all search options have been exhausted and no matches aside from alternate series then
                            # we go get the best result from that list
                            logger.fdebug(
                                '%s was a match due to alternate matching.  Continuing to search, but retaining this '
                                'result just in case.' % ComicTitle)
                            alt_match = True
                elif booktype != parsed_comic['booktype'] and ignore_booktype is False:
                    logger.fdebug('Booktypes do not match. Looking for %s, this is a %s. Ignoring this result.' % (
                        booktype, parsed_comic['booktype']))
                    continue
                else:
                ...
                if parsed_comic['series_volume'] is not None:
                    versionfound = "yes"
                    if len(parsed_comic['series_volume'][1:]) == 4 and parsed_comic['series_volume'][
                                                                       1:].isdigit():  # v2013
                    ...
                    elif len(parsed_comic['series_volume'][1:]) == 1 and parsed_comic['series_volume'][
                                                                         1:].isdigit():  # v2
                ...
                if vers4vol != "no" or vers4year != "no":
                    ...
                    logger.fdebug(
                        "Series version detected as V1 (only series in existance with that title). Bypassing "
                        "Year/Volume check")
                    yearmatch = "true"
                elif any([UseFuzzy == "0", UseFuzzy == "2", UseFuzzy is None, IssDateFix != "no"]) and parsed_comic[
                        ...
                        if str(comyear) in result_comyear:
                            ...
                            if UseFuzzy == "2":
                                ...
                                if str(ComUp) in result_comyear or str(ComDwn) in result_comyear:
                                    logger.fdebug(
                                        'Fuzzy Logic\'d the Year and got a match with a year of %s' % result_comyear)
                                    yearmatch = "true"
                                else:
                            ...
                            if IssDateFix != "no" and UseFuzzy != "2":
                                if IssDateFix == "01" or IssDateFix == "02" or IssDateFix == "03":
                                    ...
                                    if str(ComicYearFix) in result_comyear:
                                        logger.fdebug(
                                            'Further analysis reveals this was published inbetween Nov-Jan, '
                                            'decreasing year to %s has resulted in a match!' % ComicYearFix)
                                        yearmatch = "true"
                                    else:
                                    ...
                                    if str(ComicYearFix) in result_comyear:
                                        logger.fdebug(
                                            'Further analysis reveals this was published inbetween Nov-Jan, incrementing year to %s has resulted in a match!' % ComicYearFix)
                                        yearmatch = "true"
                                    else:
                                        logger.fdebug('%s - not the right year.' % comyear)
                elif UseFuzzy == "1":
                ...
                if versionfound == "yes":
                    ...
                    if F_ComicVersion == '0':
                        # need to convert dates to just be yyyy-mm-dd and do comparison, time operator in the below
                        # calc as well which probably throws off$
                        F_ComicVersion = '1'
                        if postdate_int is not None:
                            if digitaldate != '0000-00-00' and all([postdate_int >= digitaldate_int, nzbprov == '32P']):
                                logger.fdebug(
                                    '32P torrent discovery. Posting date (%s) is after DIGITAL store date (%s), '
                                    'forcing volume label to be the same as series label (0-Day Enforcement): v%s --> '
                                    'v%s' % (
                                        pubdate, digitaldate, F_ComicVersion, S_ComicVersion))
                                F_ComicVersion = D_ComicVersion
                            elif all([postdate_int >= issuedate_int, nzbprov == '32P']):
                                logger.fdebug(
                                    '32P torrent discovery. Posting date (%s) is after store date (%s), forcing volume label to be the same as series label (0-Day Enforcement): v%s --> v%s' % (
                    ...
                    if annualize == "true" and int(ComicYear) == int(F_ComicVersion):
                        logger.fdebug("We matched on versions for annuals %s" % fndcomicversion)
                            ...
                            int(F_ComicVersion) == int(D_ComicVersion) or int(F_ComicVersion) == int(S_ComicVersion)):
                        logger.fdebug("We matched on versions...%s" % fndcomicversion)
                    else:
                        if booktype == 'TPB' and int(F_ComicVersion) == int(findcomiciss) and filecomic[
                            'justthedigits'] is None:
                            logger.fdebug(
                                'TPB detected - reassigning volume %s to match as the issue number based on Volume' % fndcomicversion)
                        elif all([int(F_ComicVersion) == int(findcomiciss), fndcomicversion is not None,
                                  booktype == 'TPB', filecomic['booktype'] == 'TPB',
                                  filecomic['justthedigits'] is None]):
                            logger.fdebug(
                                'TPB detected - reassigning volume %s to match as the issue number' % fndcomicversion)
                        else:
                ...
                if (all([nzbprov == '32P', RSS == 'no', allow_packs == True]) and any(
                        [entry['pack'] == '1', entry['pack'] == '2'])) or (
                        all([nzbprov == 'ddl', pack_test is True])):  # allow_packs is True
                    if nzbprov == '32P':
                        if entry['pack'] == '2':
                            logger.fdebug('[PACK-QUEUE] Diamond FreeLeech Pack detected.')
                        elif entry['pack'] == '1':
                            logger.fdebug(
                                '[PACK-QUEUE] Normal Pack detected. Checking available inkdrops prior to downloading.')
                        else:
                    ...
                    if nowrite is False:
                        if any([nzbprov == 'dognzb', nzbprov == 'nzb.su', nzbprov == 'experimental',
                                'newznab' in nzbprov]):
                            tprov = nzbprov
                            kind = 'usenet'
                    ...
                    if filecomic['process_status'] == 'match':
                        if cmloopit != 4:
                            ...
                            if intIss is None and all([booktype == 'One-Shot',
                        ...
                        if all([intIss is not None, comintIss is not None]) and int(intIss) == int(comintIss) or all(
                                [chktpb != 0, filecomic['booktype'] == 'TPB', pc_in is None,
                                 helpers.issuedigits(F_ComicVersion) == intIss]) or all(
                            ...
                            elif all([nzbprov == 'ddl', 'getcomics' in entry['link']]) or all(
                                    [nzbprov == 'ddl', RSS == 'yes']):
                                if RSS == "yes":
                                    entry['id'] = entry['link']
                ...
                if downloadit:
                    ...
                    if nzbname is None:
                        logger.error(
                            '[NZBPROVIDER = NONE] Encountered an error using given provider with requested '
                            'information: %s. You have a blank entry most likely in your newznabs, fix it & restart '
                            'Mylar' % mylar.COMICINFO)
                        continue
                    ...
                    try:
                        ...
                        links = entry['link']
                    searchresult = searcher(nzbprov, nzbname, mylar.COMICINFO, links, IssueID, ComicID, tmpprov,
                                            newznab=newznab_host, rss=RSS)

                    if any([searchresult == 'downloadchk-fail', searchresult == 'double-pp']):
                        foundc['status'] = False
                        continue
                    elif any([searchresult == 'torrent-fail', searchresult == 'nzbget-fail', searchresult == 'sab-fail',
                              searchresult == 'blackhole-fail', searchresult == 'ddl-fail']):
                        foundc['status'] = False
                        return foundc
    ...
    if foundc['status'] is True:
        ...
        if mylar.COMICINFO[0]['pack'] is True:
            ...
            if issinfo is not None:
                # we need to get EVERY issue ID within the pack and update the log to reflect that they're being
                # downloaded via a pack.

                try:
                    logger.fdebug(
                        'Found matching comic within pack...preparing to send to Updater with IssueIDs: %s and '
                        'nzbname of %s' % (
                            issueid_info, nzbname))
                except NameError:
                    logger.fdebug('Did not find issueid_info')

                # because packs need to have every issue that's not already Downloaded in a Snatched status,
                # throw it to the updater here as well.
                for isid in issinfo['issues']:
                    updater.nzblog(isid['issueid'], nzbname, ComicName, SARC=SARC, IssueArcID=IssueArcID, id=nzbid,
                                   prov=tmpprov, oneoff=oneoff)
                    updater.foundsearch(ComicID, isid['issueid'], mode='series', provider=tmpprov)
                notify_snatch(sent_to, mylar.COMICINFO[0]['ComicName'], mylar.COMICINFO[0]['comyear'],
                              mylar.COMICINFO[0]['pack_numbers'], nzbprov, True)
            else:
                notify_snatch(sent_to, mylar.COMICINFO[0]['ComicName'], mylar.COMICINFO[0]['comyear'], None, nzbprov,
                              True)

        else:
            if alt_nzbname is None or alt_nzbname == '':
                logger.fdebug('Found matching comic...preparing to send to Updater with IssueID: %s and nzbname: %s' % (
                    IssueID, nzbname))
                ...
                logger.fdebug(
                    'Found matching comic...preparing to send to Updater with IssueID: %s and nzbname: %s [%s]' % (
                        IssueID, nzbname, alt_nzbname))
                if '[RSS]' in tmpprov: tmpprov = re.sub('\[RSS\]', '', tmpprov).strip()
                updater.nzblog(IssueID, nzbname, ComicName, SARC=SARC, IssueArcID=IssueArcID, id=nzbid, prov=tmpprov,
                               alt_nzbname=alt_nzbname, oneoff=oneoff)
...
def searchforissue(issueid=None, rsscheck=None, manual=False):
    if rsscheck == 'yes':
    ...
    try:
        ...
        if (any([mylar.CONFIG.ENABLE_DDL is True, mylar.CONFIG.NZBSU is True, mylar.CONFIG.DOGNZB is True,
                 mylar.CONFIG.EXPERIMENTAL is True]) or all([mylar.CONFIG.NEWZNAB is True, len(ens) > 0]) and any(
            [mylar.USE_SABNZBD is True, mylar.USE_NZBGET is True, mylar.USE_BLACKHOLE is True])) or (
                all([mylar.CONFIG.ENABLE_TORRENT_SEARCH is True, mylar.CONFIG.ENABLE_TORRENTS is True]) and (
                any([mylar.CONFIG.ENABLE_PUBLIC is True, mylar.CONFIG.ENABLE_32P is True]) or all(
            ...
            if not issueid or rsscheck:
                        ...
                        if mylar.CONFIG.SEARCH_STORYARCS is True or rsscheck:
                            if mylar.CONFIG.FAILED_DOWNLOAD_HANDLING and mylar.CONFIG.FAILED_AUTO:
                                issues_2 = myDB.select(
                                    'SELECT * from storyarcs WHERE Status="Wanted" OR Status="Failed"')
                            else:
                ...
                for result in sorted(results, key=itemgetter('StoreDate'), reverse=True):
                    ...
                    comic = myDB.selectone("SELECT * from comics WHERE ComicID=? AND ComicName != 'None'",
                                           [result['ComicID']]).fetchone()
                    if all([comic is None, result['mode'] == 'story_arc']):
                        comic = myDB.selectone("SELECT * from storyarcs WHERE StoryArcID=? AND IssueArcID=?",
                                               [result['StoryArcID'], result['IssueArcID']]).fetchone()
                        if comic is None:
                            logger.fdebug(
                                '%s has no associated comic information in the Arc. Skipping searching for this series.' %
                                result['ComicID'])
                            continue
                        else:
                            OneOff = True
                    ...
                    if result['StoreDate'] == '0000-00-00' or result['StoreDate'] is None:
                        if any([result['IssueDate'] is None, result['IssueDate'] == '0000-00-00']) and result[
                            'DigitalDate'] == '0000-00-00':
                            logger.fdebug(
                                'ComicID: %s has invalid Date data. Skipping searching for this series.' % result[
                                    'ComicID'])
                            continue
                    ...
                    if result['DateAdded'] is None:
                        ...
                        if result['mode'] == 'want':
                            ...
                            table = 'storyarcs'
                        logger.fdebug('%s #%s did not have a DateAdded recorded, setting it : %s' % (
                            comic['ComicName'], result['Issue_Number'], DateAdded))
                    ...
                    if rsscheck is None and DateAdded >= mylar.SEARCH_TIER_DATE:
                        ...
                        mylar.SEARCH_QUEUE.put({'comicname': comic['ComicName'], 'seriesyear': SeriesYear,
                                                'issuenumber': result['Issue_Number'], 'issueid': result['IssueID'],
                                                'comicid': result['ComicID'], 'booktype': booktype})
                        continue

                    mode = result['mode']
                    foundNZB, prov = search_init(comic['ComicName'], result['Issue_Number'], str(ComicYear), SeriesYear,
                                                 Publisher, IssueDate, StoreDate, result['IssueID'], AlternateSearch,
                                                 UseFuzzy, ComicVersion, SARC=result['SARC'],
                                                 IssueArcID=result['IssueArcID'], mode=mode, rsscheck=rsscheck,
                                                 ComicID=result['ComicID'], filesafe=Comicname_filesafe,
                                                 allow_packs=AllowPacks, oneoff=OneOff, torrentid_32p=TorrentID_32p,
                                                 digitaldate=DigitalDate, booktype=booktype,
                    ...
                    if foundNZB['status'] is True:
                        updater.foundsearch(result['ComicID'], result['IssueID'], mode=mode, provider=prov,
                                            SARC=result['SARC'], IssueArcID=result['IssueArcID'],
                ...
                foundNZB, prov = search_init(ComicName, IssueNumber, str(IssueYear), SeriesYear, Publisher, IssueDate,
                                             StoreDate, actissueid, AlternateSearch, UseFuzzy, ComicVersion, SARC=SARC,
                                             IssueArcID=IssueArcID, mode=mode, rsscheck=rsscheck, ComicID=ComicID,
                                             filesafe=Comicname_filesafe, allow_packs=allow_packs, oneoff=oneoff,
                                             manual=manual, torrentid_32p=TorrentID_32p, digitaldate=DigitalDate,
                                             booktype=booktype, ignore_booktype=ignore_booktype)
                if manual is True:
                    mylar.SEARCHLOCK = False
                    return foundNZB
                if foundNZB['status'] is True:
                    logger.fdebug('I found %s #%s' % (ComicName, IssueNumber))
                    updater.foundsearch(ComicID, actissueid, mode=mode, provider=prov, SARC=SARC, IssueArcID=IssueArcID,
                                        hash=foundNZB['info']['t_hash'])
                if mylar.SEARCHLOCK is True:
                    mylar.SEARCHLOCK = False
                return foundNZB
            ...
            if rsscheck:
                logger.warn(
                    'There are no search providers enabled atm - not performing an RSS check for obvious reasons')
            else:
                logger.warn(
                    'There are no search providers enabled atm - not performing an Force Check for obvious reasons')
        return
    ...
    if (any([mylar.CONFIG.ENABLE_DDL is True, mylar.CONFIG.NZBSU is True, mylar.CONFIG.DOGNZB is True,
        ...
        for issueid in issuelist:
            ...
            if issue is None:
                ...
                if issue is None:
                    logger.warn(
                        'Unable to determine IssueID - perhaps you need to delete/refresh series? Skipping this '
                        'entry: %s' % issueid)
                    continue

            if any([issue['Status'] == 'Downloaded', issue['Status'] == 'Snatched']):
                logger.fdebug(
                    'Issue is already in a Downloaded / Snatched status. If this is still wanted, perform a Manual '
                    'search or mark issue as Skipped or Wanted.')
                continue
            ...
            mylar.SEARCH_QUEUE.put(
                {'comicname': comic['ComicName'], 'seriesyear': SeriesYear, 'issuenumber': issue['Issue_Number'],
                 'issueid': issue['IssueID'], 'comicid': issue['ComicID'], 'booktype': booktype})
...
def provider_sequence(nzbprovider, torprovider, newznab_hosts, torznab_hosts, ddlprovider):
    ...
    if len(mylar.CONFIG.PROVIDER_ORDER) > 0:
        for pr_order in sorted(list(mylar.CONFIG.PROVIDER_ORDER.items()), key=itemgetter(0), reverse=False):
            if any(pr_order[1].lower() in y for y in torproviders_lower) or any(
                    pr_order[1].lower() in x for x in nzbproviders_lower) or any(
                pr_order[1].lower() == z for z in ddlproviders_lower):
...
def searcher(nzbprov, nzbname, comicinfo, link, IssueID, ComicID, tmpprov, directsend=None, newznab=None, rss=None):
    alt_nzbname = None
    ...
    if mylar.CONFIG.NZBGET_PRIORITY:
        if mylar.CONFIG.NZBGET_PRIORITY == "Default":
            pass
        elif mylar.CONFIG.NZBGET_PRIORITY == "Low":
            pass
        elif mylar.CONFIG.NZBGET_PRIORITY == "Normal":
            pass
    ...
    if comicinfo[0]['pack']:
        if '0-Day Comics Pack' not in comicinfo[0]['ComicName']:
            logger.info('Found %s (%s) issue: %s using %s within a pack containing issues %s' % (
                ComicName, comyear, IssueNumber, tmpprov, comicinfo[0]['pack_numbers']))
        else:
    ...
    if mylar.CONFIG.FAILED_DOWNLOAD_HANDLING:
        ...
        if all([nzbid is not None and nzbid != '', IssueID is not None, oneoff is False]):
            ...
            # except:
            #    pass
            call_the_fail = Failed.FailedProcessor(nzb_name=nzbname, id=nzbid, issueid=IssueID, comicid=ComicID,
                                                   prov=tmpprov)
            ...
            if check_the_fail == 'Failed':
                ...
                logger.fdebug(
                    '[FAILED_DOWNLOAD_CHECKER] This is not in the failed downloads list. Will continue with the download.')
        else:
            logger.fdebug(
                '[FAILED_DOWNLOAD_CHECKER] Failed download checking is not available for one-off downloads atm. Fixed '
                'soon!')
    ...
    if link and all([nzbprov != 'WWT', nzbprov != 'DEM', nzbprov != '32P', nzbprov != 'torznab', nzbprov != 'ddl']):
        ...
        if filen is None:
            try:
                filen = r.headers['content-disposition'][
                        r.headers['content-disposition'].index("filename=") + 9:].strip(';').strip('"')
        ...
        if filen is None:
            ...
            if re.sub('.nzb', '', filen.lower()).strip() != re.sub('.nzb', '', nzbname.lower()).strip():
                ...
                logger.info('filen: %s -- nzbname: %s are not identical. Storing extra value as : %s' % (
                    filen, nzbname, alt_nzbname))

            # make sure the cache directory exists - if not, create it (used for storing nzbs).
            if os.path.exists(mylar.CONFIG.CACHE_DIR):
                if mylar.CONFIG.ENFORCE_PERMS:
                    logger.fdebug(
                        'Cache Directory successfully found at : %s. Ensuring proper permissions.' % mylar.CONFIG.CACHE_DIR)
    ...
    if mylar.CONFIG.ENABLE_DDL is True and nzbprov == 'ddl':
        ...
        if ddl_it['success'] is True:
            logger.info(
                'Successfully snatched %s from DDL site. It is currently being queued to download in position %s' % (
                    nzbname, mylar.DDL_QUEUE.qsize()))
        else:
        ...
        if os.path.exists(mylar.CONFIG.BLACKHOLE_DIR):
            ...
            logger.info(
                'Successfully sent .nzb to your Blackhole directory : %s' % os.path.join(mylar.CONFIG.BLACKHOLE_DIR,
                                                                                         nzbname))
    ...
    # torrents (32P & DEM)
        ...
        rcheck = rsscheck.torsend2client(ComicName, IssueNumber, comyear, link, nzbprov,
                                         nzbid)  # nzbid = hash for usage with public torrents
        if rcheck == "fail":
            if mylar.CONFIG.FAILED_DOWNLOAD_HANDLING:
                logger.error(
                    'Unable to send torrent to client. Assuming incomplete link - sending to Failed Handler and continuing search.')
                if any([oneoff is True, IssueID is None]):
                    logger.fdebug(
                ...
                return FailedMark(prov=nzbprov)
            else:
                logger.error(
                    'Unable to send torrent - check logs and settings (this would be marked as a BAD torrent if '
                    'Failed Handling was enabled)')
                return "torrent-fail"
        else:
                ...
                if mylar.CONFIG.ENABLE_SNATCH_SCRIPT:
                    try:
                        if comicinfo[0]['pack'] is False:
                            ...
                            if '0-Day Comics Pack' in ComicName:
                                helpers.lookupthebitches(rcheck['files'], rcheck['folder'], nzbname, nzbid, nzbprov,
                                                         t_hash, comicinfo[0]['IssueDate'])
                                pnumbers = None
                                plist = None
                            else:
                                pnumbers = '|'.join(comicinfo[0]['pack_numbers'])
        ...
        # end nzb.get
            ...
            if mylar.LOCAL_IP is None:
                ...
                try:
                    ...
                    logger.warn(
                        'Unable to determine local IP. Defaulting to host address for Mylar provided as : %s' % mylar.CONFIG.HTTP_HOST)
            ...
            if mylar.CONFIG.HOST_RETURN:
                ...
                # if sab & mylar are on different machines, check to see if they are local or external IP's provided
                # for host.
                if mylar.CONFIG.HTTP_HOST == 'localhost' or mylar.CONFIG.HTTP_HOST == '0.0.0.0' or mylar.CONFIG.HTTP_HOST.startswith(
                        '10.') or mylar.CONFIG.HTTP_HOST.startswith('192.') or mylar.CONFIG.HTTP_HOST.startswith(
                    '172.'):
                    ...
                    if mylar.EXT_IP is None:
                        ...
                        try:
                            ...
                            logger.warn(
                                'Unable to retrieve External IP - try using the host_return option in the config.ini.')
            ...
            if sab_params is not None:
                ...
                logger.warn(
                    'Unable to send nzb file to SABnzbd. There was a parameter error as there are no values present: %s' % sab_params)
    ...
    if directsend is None:
        ...
        if alt_nzbname is None or alt_nzbname == '':
            logger.fdebug(
                "Found matching comic...preparing to send to Updater with IssueID %s and nzbname of %s [Oneoff:%s]" % (
            ...
            logger.fdebug(
                "Found matching comic...preparing to send to Updater with IssueID %s and nzbname of %s ["
                "ALTNZBNAME:%s][OneOff:%s]" % (
                    IssueID, nzbname, alt_nzbname, oneoff))
            if '[RSS]' in tmpprov: tmpprov = re.sub('\[RSS\]', '', tmpprov).strip()
            updater.nzblog(IssueID, nzbname, ComicName, SARC=SARC, IssueArcID=IssueArcID, id=nzbid, prov=tmpprov,
                           alt_nzbname=alt_nzbname, oneoff=oneoff)
        # send out notifications for on snatch after the updater incase notification fails (it would bugger up the
        # updater/pp scripts)
...
def FailedMark(prov):
    # Used to pass a failed attempt at sending a download to a client, to the failed handler, and then back again to
    # continue searching.
...
def IssueTitleCheck(issuetitle, watchcomic_split, splitit, splitst, issue_firstword, hyphensplit, orignzb=None):
    ...
    if (splitst - 1) > len(watchcomic_split):
        ...
        if iss_calc >= 80:  # mylar.ISSUE_TITLEMATCH - user-defined percentage to match against for issue name
            # comparisons.
